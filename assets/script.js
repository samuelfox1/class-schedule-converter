...
formEl.on('submit', (e) => {
    ...
    const config = {
        ...
        adjustTime: {
            ...
            breakLength: parseInt(breakTime)
        },
    }

    console.log(config)
    // convertStartTime(startingHour, startingMinute, startingAmPm)
    convertStartTime(config)

    // console.log('\n\noutput:\n', reassembleSchedule())
    scheduleOutputEl.text(reassembleSchedule())

})

const init = () => {
    scheduleInputEl.text(string.trim());
    timeInputEl.val('18:30');
}
init()